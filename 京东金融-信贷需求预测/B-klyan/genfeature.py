...
def getNearestClick(df, duser, tuser, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            uid_click_interval["click_nearest_interval"] = (
                    pd.Timestamp(valid_end_date) - uid_click_interval["click_time"]).apply(lambda x: x.total_seconds())
        elif idx == 1:
            uid_click_interval["click_nearest_interval"] = (
                    pd.Timestamp(test_end_date) - uid_click_interval["click_time"]).apply(lambda x: x.total_seconds())
        uid_click_interval["next_click"] = uid_click_interval["click_interval"] + uid_click_interval[
            "click_nearest_interval"]
...
def uid_order_status(df, duser, tuser, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        uid_order["mean_order_amt_percent_mean_buy_cnt_percent"] = uid_order["mean_order_amt_percent"] * uid_order[
            "mean_buy_cnt_percent"]
        uid_order.columns = ['uid', 'order_amt' + str(window_size), 'buy_cnt' + str(window_size),
                             "avg_order_amt" + str(window_size), 'mean_order_amt_percent' + str(window_size),
                             'mean_buy_cnt_percent' + str(window_size),
                             'mean_order_amt_percent_mean_buy_cnt_percent' + str(window_size)]
        if idx == 0:
...
def getNearestOrder(df, duser, tuser, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            uid_nearest_buy["buy_nearest_interval"] = (
                    pd.Timestamp(valid_end_date) - uid_nearest_buy["buy_time"]).apply(lambda x: x.days)
        elif idx == 1:
            uid_nearest_buy["buy_nearest_interval"] = (pd.Timestamp(test_end_date) - uid_nearest_buy["buy_time"]).apply(
                lambda x: x.days)
        uid_nearest_buy["next_buytime"] = uid_nearest_buy["buy_interval"] + uid_nearest_buy["buy_nearest_interval"]
        uid_nearest_buy["buy_nearest_price_interval"] = uid_nearest_buy["order_amt"] / (
                uid_nearest_buy["buy_nearest_interval"] + 1)
        # uid_nearest_buy.drop(["buy_interval"],axis=1,inplace=True)
        uid_nearest_buy.columns = ["uid", "mean_buy_interval", "nearest_buytime", "buy_nearest_price",
                                   "buy_nearest_interval", "next_buytime", "buy_nearest_price_interval"]
        if idx == 0:
            duser = duser.merge(uid_nearest_buy, how="left", on="uid")
            duser["nearest_buytime_to_active_date"] = (duser["nearest_buytime"] - duser["active_date"]).apply(
                lambda x: x.days)  # 最近一次购买距离用户激活的时间
            duser.drop("nearest_buytime", inplace=True, axis=1)
        elif idx == 1:
            tuser = tuser.merge(uid_nearest_buy, how="left", on="uid")
            tuser["nearest_buytime_to_active_date"] = (tuser["nearest_buytime"] - tuser["active_date"]).apply(
                lambda x: x.days)  # 最近一次购买距离用户激活的时间
...
def getMaxPriceOrder(df, duser, tuser, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            uid_max_buy["maxbuy_interval"] = (pd.Timestamp(valid_end_date) - uid_max_buy["buy_time"]).apply(
                lambda x: x.days)
        elif idx == 1:
            uid_max_buy["maxbuy_interval"] = (pd.Timestamp(test_end_date) - uid_max_buy["buy_time"]).apply(
                lambda x: x.days)
...
def gen_fixedtw_features_for_loan(df, duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        stat_loanPlanNum.columns = ['uid'] + [i + '_loanPlanNum_' + str(window_size) for i in
                                              list(stat_loanPlanNum.columns)[1:]]
        # 每期贷款额 amt_per_plan
        stat_amtPerPlan = tmp.groupby(["uid"])['amt_per_plan'].agg(['sum', 'mean', 'max', 'min']).reset_index()
        stat_amtPerPlan.columns = ['uid'] + [i + '_amtPerPlan_' + str(window_size) for i in
                                             list(stat_amtPerPlan.columns)[1:]]
        # 频率最高的贷款期数和对应的贷款次数
        ...
        stat_loanInterval.columns = ['uid'] + [i + '_loanInterval_' + str(window_size) for i in
                                               list(stat_loanInterval.columns)[1:]]
        loan_stat = stat_loanAmt.merge(stat_loanPlanNum, how="left", on="uid").merge(stat_amtPerPlan, how="left",
                                                                                     on="uid").merge(stat_loanInterval,
                                                                                                     how="left",
                                                                                                     on="uid").merge(
            most_freq, how="left", on="uid").merge(perPlanAmtCnt, how="left", on="uid")
        if idx == 0:
            duser1 = duser1.merge(loan_stat, how="left", on="uid")
            duser1[new_list] = duser1[new_list].fillna(0.0)
            stat_daysLoan = (pd.Timestamp(valid_end_date) - tmp.loan_time).apply(
                lambda x: x.days + x.seconds / 86400.0).groupby(tmp.uid).agg(
                ['mean', 'max', 'min']).reset_index()  # 各次贷款离现在的时间相关的统计
            stat_daysLoan.columns = ['uid'] + [i + '_nearestLoanInterval_' + str(window_size) for i in
                                               list(stat_daysLoan.columns)[1:]]
            ...
            stat_daysLoan = (pd.Timestamp(test_end_date) - tmp.loan_time).apply(
                lambda x: x.days + x.seconds / 86400.0).groupby(tmp.uid).agg(['mean', 'max', 'min']).reset_index()
            stat_daysLoan.columns = ['uid'] + [i + '_nearestLoanInterval_' + str(window_size) for i in
                                               list(stat_daysLoan.columns)[1:]]
...
def getNearestLoan(df, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            uid_nearest_loan['nearest_loantime'] = (pd.Timestamp(valid_end_date) - uid_nearest_loan.loan_time).apply(
                lambda x: x.days + x.seconds / 86400.0)
            uid_nearest_loan['nearest_loan_amt_time'] = uid_nearest_loan['loan_amount'] / (
                    1 + uid_nearest_loan['nearest_loantime'])
            uid_nearest_loan = uid_nearest_loan[
                ["uid", "plannum", "amt_per_plan", "loan_amount", "nearest_loan_amt_time", "nearest_loantime"]]
            uid_nearest_loan.columns = ["uid", "nearest_plannum", "nearest_amt_per_plan", "nearest_loan_amount",
                                        "nearest_loan_amt_time", "nearest_loantime"]
            duser1 = duser1.merge(uid_nearest_loan, how="left", on="uid")
        elif idx == 1:
            uid_nearest_loan['nearest_loantime'] = (pd.Timestamp(test_end_date) - uid_nearest_loan.loan_time).apply(
                lambda x: x.days + x.seconds / 86400.0)
            uid_nearest_loan['nearest_loan_amt_time'] = uid_nearest_loan['loan_amount'] / (
                    1 + uid_nearest_loan['nearest_loantime'])
            uid_nearest_loan = uid_nearest_loan[
...
def currentDebtAmt(df, duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        if idx == 0:
            ...
            current_debtAmt = tmp.groupby("uid")["debtAmt"].sum().rename(
                "current_debtAmt_" + str(window_size)).reset_index()
...
def avgLoanAmt4orderAmt(df_loan, df_order, duser1, duser2):
    ...
    month_orderAmt = \
        df_order_tmp.groupby(["uid", "month"])["order_amt"].sum().rename("uid_month_orderAmt").reset_index().groupby(
            "uid")[
            "uid_month_orderAmt"].mean().rename("uid_avg_month_orderAmt").reset_index()
    month_loanAmt = \
        df_loan_tmp.groupby(["uid", "month"])["loan_amount"].sum().rename("uid_month_loanAmt").reset_index().groupby(
            "uid")[
            "uid_month_loanAmt"].mean().rename("uid_avg_month_loanAmt").reset_index()
    month_order_loan = month_orderAmt.merge(month_loanAmt, on="uid", how="left").fillna(0.0)
    month_order_loan["loanAmt_ratio"] = month_order_loan["uid_avg_month_loanAmt"] / month_order_loan[
        "uid_avg_month_orderAmt"]
...
def getLoanAmtRemainingLimt(df_loan, duser1, duser2, before_month):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        uid_month_loanamt = df_loan[mask].groupby("uid")["loan_amount"].sum().rename(
            "month_sum_loanamt" + str(before_month)).reset_index()  # 分子
        if idx == 0:
            ...
            current_debtAmt = tmp.groupby("uid")["debtAmt"].sum().rename(
                "debtAmt_monthBefore" + str(before_month)).reset_index()
            ...
            duser1["debtAmt_monthBefore" + str(before_month)] = duser1[
                "debtAmt_monthBefore" + str(before_month)].fillna(0.0)
            duser1["remainingAmt_monthBefore" + str(before_month)] = duser1["limit"] - duser1[
                "debtAmt_monthBefore" + str(before_month)]
            duser1["loansum_remainingAmt_ratio_monthBefore" + str(before_month)] = duser1["month_sum_loanamt" + str(
                before_month)] / (1 + duser1["remainingAmt_monthBefore" + str(before_month)])
        elif idx == 1:
            tmp = df_loan[(df_loan["loan_time"] < test_start) & (df_loan["pay_end_date"] > test_start)].reset_index(
                drop=True)
            ...
            duser2["debtAmt_monthBefore" + str(before_month)] = duser2[
                "debtAmt_monthBefore" + str(before_month)].fillna(0.0)
            duser2["remainingAmt_monthBefore" + str(before_month)] = duser2["limit"] - duser2[
                "debtAmt_monthBefore" + str(before_month)]
            duser2["loansum_remainingAmt_ratio_monthBefore" + str(before_month)] = duser2["month_sum_loanamt" + str(
                before_month)] / (1 + duser2["remainingAmt_monthBefore" + str(before_month)])
    return duser1, duser2
...
def getAvailableLoanAmtLimt(df_loan, duser1, duser2, before_month):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        if idx == 0:
            ...
            tmp = df_loan[debt_mask].reset_index(drop=True)
            tmp["debtRatio"] = (tmp["pay_end_date"] - valid_end).apply(
                lambda x: 1 if x.days / 30.0 >= 1 else x.days / 30.0)
            tmp["unAvailableAmt"] = tmp["loan_amount"] * tmp["debtRatio"]
            unAvailableAmt = tmp.groupby("uid")["unAvailableAmt"].sum().rename(
                "unAvailableAmt_monthBefore" + str(before_month)).reset_index()
            duser1 = duser1.merge(unAvailableAmt, on="uid", how="left")  # 当月不可用额度
            duser1["unAvailableAmt_monthBefore" + str(before_month)] = duser1[
                "unAvailableAmt_monthBefore" + str(before_month)].fillna(0.0)
            duser1["availableAmt_monthBefore" + str(before_month)] = duser1["limit"] - duser1[
                "unAvailableAmt_monthBefore" + str(before_month)]
        elif idx == 1:
            tmp = df_loan[(df_loan["loan_time"] < test_end) & (df_loan["pay_end_date"] > test_end)].reset_index(
                drop=True)
            tmp["debtRatio"] = (tmp["pay_end_date"] - test_end).apply(
                lambda x: 1 if x.days / 30.0 >= 1 else x.days / 30.0)
            tmp["unAvailableAmt"] = tmp["loan_amount"] * tmp["debtRatio"]
            unAvailableAmt = tmp.groupby("uid")["unAvailableAmt"].sum().rename(
                "unAvailableAmt_monthBefore" + str(before_month)).reset_index()
            duser2 = duser2.merge(unAvailableAmt, on="uid", how="left")  # 当月不可用额度
            duser2["unAvailableAmt_monthBefore" + str(before_month)] = duser2[
                "unAvailableAmt_monthBefore" + str(before_month)].fillna(0.0)
            duser2["availableAmt_monthBefore" + str(before_month)] = duser2["limit"] - duser2[
                "unAvailableAmt_monthBefore" + str(before_month)]
    return duser1, duser2
...
def lastMonthPayedAmt(df, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            tmp["last_month_topay_num"] = tmp["pay_end_date"].apply(
                lambda x: 1 if (x - valid_start).days / 30.0 >= 1 else (x - valid_start).days / 30.0)
            tmp["last_month_payed_amt"] = tmp["amt_per_plan"] * tmp["last_month_topay_num"]
            lastMonthPayedAmt = tmp.groupby("uid")["last_month_payed_amt"].sum().rename(
                "lastMonthPayedAmt").reset_index()
            duser1 = duser1.merge(lastMonthPayedAmt, on="uid", how="left")
            duser1["lastMonthPayedAmt"] = duser1["lastMonthPayedAmt"].fillna(0.0)
        elif idx == 1:
            tmp["last_month_topay_num"] = tmp["pay_end_date"].apply(
                lambda x: 1 if (x - test_start).days / 30.0 >= 1 else (x - test_start).days / 30.0)
            tmp["last_month_payed_amt"] = tmp["amt_per_plan"] * tmp["last_month_topay_num"]
            lastMonthPayedAmt = tmp.groupby("uid")["last_month_payed_amt"].sum().rename(
                "lastMonthPayedAmt").reset_index()
...
def getPast3MonthLoanFeatures(df, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        # 平均每月贷款金额
        month_loanAmt = \
            tmp.groupby(["uid", "month"])['loan_amount'].sum().rename("monthLoanAmt").reset_index().groupby(["uid"])[
                "monthLoanAmt"].mean().rename("monthAvgLoanAmt").reset_index()
        ...
        stat_loanInterval.columns = ['uid'] + [i + '_loanInterval_' + str(window_size) for i in
                                               list(stat_loanInterval.columns)[1:]]
        loan3Month = month_loanAmt.merge(stat_loanAmt, on="uid", how="left").merge(stat_loanPlanNum, on="uid",
                                                                                   how="left").merge(stat_amtPerPlan,
                                                                                                     on="uid",
                                                                                                     how="left").merge(
            stat_loanInterval, on="uid", how="left").fillna(0.0)
        if idx == 0:
...
def loanTimeBetweenActivetime(df, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        tmp = df[mask]
        uid_nearest_loan = tmp[tmp.groupby(['uid'])['loan_time'].transform(max) == tmp['loan_time']][
            ["uid", "loan_time", "loan_amount"]]  # 用户最近一次借贷的情况
        uid_first_loan = tmp[tmp.groupby(['uid'])['loan_time'].transform(min) == tmp['loan_time']][
            ["uid", "loan_time", "loan_amount"]]  # 用户第一天借贷的情况
        ...
        if idx == 0:
            ...
            duser1["first_loantime_active_days"] = (duser1["active_date"] - duser1["first_loan_time"]).apply(
                lambda x: x.days)  # 第一次借贷距离用户激活的时间
            duser1["first_loan_amount_limit"] = duser1["first_loan_amt"] / duser1["limit"]
            duser1["nearest_loantime_active_days"] = (duser1["active_date"] - duser1["nearest_loan_time"]).apply(
                lambda x: x.days)  # 最近一次借贷距离用户激活的时间
            ...
            duser2["first_loantime_active_days"] = (duser2["active_date"] - duser2["first_loan_time"]).apply(
                lambda x: x.days)  # 第一次借贷距离用户激活的时间
            duser2["first_loan_amount_limit"] = duser2["first_loan_amt"] / duser2["limit"]
            duser2["nearest_loantime_active_days"] = (duser2["active_date"] - duser2["nearest_loan_time"]).apply(
                lambda x: x.days)  # 最近一次借贷距离用户激活的时间
...
def getOrderClickRatio(df_click, df_order, duser1, duser2):
    ...
    uid_valid_clicks = df_click[click_valid_mask].groupby("uid")["click_time"].count().rename(
        "total_clicks_3month").reset_index()
    uid_test_clicks = df_click[click_test_mask].groupby("uid")["click_time"].count().rename(
        "total_clicks_3month").reset_index()
    uid_valid_orders = df_order[order_valid_mask].groupby("uid")["buy_time"].count().rename(
        "total_order_3month").reset_index()
    uid_test_orders = df_order[order_test_mask].groupby("uid")["buy_time"].count().rename(
        "total_order_3month").reset_index()
    ...
    uid_valid_click_order["click_order_ratio"] = uid_valid_click_order["total_clicks_3month"] / (
            uid_valid_click_order["total_order_3month"] + 1)
    uid_test_click_order["click_order_ratio"] = uid_test_click_order["total_clicks_3month"] / (
            uid_test_click_order["total_order_3month"] + 1)
...
def getNearest2LoanInterval(df_loan, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        tmp = df_loan[mask]
        nearestLastLoanInterval = tmp[tmp.groupby(['uid'])['loan_time'].transform(max) == tmp['loan_time']][
            ["uid", "loan_interval"]]  # 用户最近一次借贷的前一次借贷间隔
...
def userDiscountRatio(df_order, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        orderAmtDiscount["discount_ratio"] = 1 - orderAmtDiscount["total_discount_amt"] / orderAmtDiscount[
            "total_order_amt"]
        if idx == 0:
...
def getMonthLoanShiftDiff(df_loan, duser1, duser2):
    ...
    test_mask = df_loan.month.isin([8, 9, 10, 11])
    for idx, mask in enumerate([valid_mask, test_mask]):
        uidMonthLoan = df_loan[mask].groupby(["uid", "month"])["loan_amount"].sum().rename(
            "month_loan_amt").reset_index()
        ...
        uidMonthLoan = uidMonthLoan.groupby(["uid"]).apply(
            lambda x: x.sort_values(["month"], ascending=True)).reset_index(drop=True)
        uidMonthLoan["monthLoanAmtDiff"] = uidMonthLoan.groupby("uid")["month_loan_amt"].apply(lambda x: x - x.shift(1))
        uidMonthLoan["monthLoanAmtDiff2"] = uidMonthLoan.groupby("uid")["monthLoanAmtDiff"].apply(
            lambda x: x - x.shift(1))
        if idx == 0:
            duser1 = duser1.merge(
                uidMonthLoan[uidMonthLoan.month == 10][["uid", "monthLoanAmtDiff", "monthLoanAmtDiff2"]], on="uid",
                how="left")
        elif idx == 1:
            duser2 = duser2.merge(
                uidMonthLoan[uidMonthLoan.month == 11][["uid", "monthLoanAmtDiff", "monthLoanAmtDiff2"]], on="uid",
                how="left")
    return duser1, duser2
...
def getMonthOrderShiftDiff(df_order, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        uidMonthLoan = df_order[mask].groupby(["uid", "month"])["order_amt"].sum().rename(
            "month_order_amt").reset_index()
        ...
        uidMonthLoan = uidMonthLoan.groupby(["uid"]).apply(
            lambda x: x.sort_values(["month"], ascending=True)).reset_index(drop=True)
        uidMonthLoan["monthOrderAmtDiff"] = uidMonthLoan.groupby("uid")["month_order_amt"].apply(
            lambda x: x - x.shift(1))
        uidMonthLoan["monthOrderAmtDiff2"] = uidMonthLoan.groupby("uid")["monthOrderAmtDiff"].apply(
            lambda x: x - x.shift(1))
        if idx == 0:
            duser1 = duser1.merge(
                uidMonthLoan[uidMonthLoan.month == 10][["uid", "monthOrderAmtDiff", "monthOrderAmtDiff2"]], on="uid",
                how="left")
        elif idx == 1:
            duser2 = duser2.merge(
                uidMonthLoan[uidMonthLoan.month == 11][["uid", "monthOrderAmtDiff", "monthOrderAmtDiff2"]], on="uid",
                how="left")
    return duser1, duser2
...
def getLoanCntShiftDiff(df_loan, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            duser1 = duser1.merge(uidMonthLoan.loc[uidMonthLoan.month == 10, ["uid", "loanCntDiff", "loanCntDiff2"]],
                                  on="uid", how="left")
        elif idx == 1:
            duser2 = duser2.merge(uidMonthLoan.loc[uidMonthLoan.month == 11, ["uid", "loanCntDiff", "loanCntDiff2"]],
                                  on="uid", how="left")
    return duser1, duser2
...
def updateLimit(duser1, duser2):
    ...
    for idx, mask in enumerate([loan_validmask, loan_testmask]):
        ...
        if idx == 0:
            ...
            duser1["limit"] = duser1.apply(lambda x: x["new_limit"] if x["limit"] < x["new_limit"] else x["limit"],
                                           axis=1)
            duser1.drop("new_limit", axis=1, inplace=True)
        elif idx == 1:
            duser2 = duser2.merge(updateLimit, on="uid", how="left")
            duser2["limit"] = duser2.apply(lambda x: x["new_limit"] if x["limit"] < x["new_limit"] else x["limit"],
                                           axis=1)
...
def updateLimit2(duser1, duser2):
    ...
    loan_testmask = (t_loan.month < 12)
    for idx, mask in enumerate([loan_validmask, loan_testmask]):
        uid_newlimit = t_loan[mask & (t_loan.plannum > 6)].groupby(["uid", "month"])["loan_amount"].sum().rename(
            "loan12amt").reset_index().groupby("uid")["loan12amt"].max().rename("new_limit").reset_index()
        uid_day_loan = \
            t_loan[mask].groupby(["uid", "date"])["loan_amount"].sum().rename("new_limit").reset_index().groupby("uid")[
                "new_limit"].max().rename("new_limit").reset_index()
        uid_newlimit_all = pd.concat([uid_newlimit, uid_day_loan]).groupby("uid")["new_limit"].max().rename(
            "new_limit").reset_index()
...
def getAmtBeforeRatio(df, column, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            ...
            mean_before_month = uid_months[uid_months.month < 10].groupby("uid")[column + '_sum'].mean().rename(
                "mean_before_month").reset_index()
            ...
            mean_before_month = uid_months[uid_months.month < 11].groupby("uid")[column + '_sum'].mean().rename(
                "mean_before_month").reset_index()
            uid_valid = uid_valid.merge(mean_before_month, how="left", on="uid")
            uid_valid[column + '_sum_before_ratio'] = uid_valid[column + '_sum'] / (uid_valid['mean_before_month'] + 1)
        if idx == 0:
...
def currentMinDebtAmt(df, duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        if idx == 0:
            pay_date_mask = df["pay_end_date"] > pd.Timestamp(valid_end_date)
            tmp = df[mask & pay_date_mask].reset_index(drop=True)
            tmp["payed_num"] = (pd.Timestamp(valid_end_date) - tmp["loan_time"]).apply(
                lambda x: math.ceil(x.days / 30.0))
            tmp["debtAmt"] = tmp["loan_amount"] - tmp["amt_per_plan"] * tmp["payed_num"]
            current_debtAmt = tmp.groupby("uid")["debtAmt"].sum().rename(
                "current_MindebtAmt_" + str(window_size)).reset_index()
            duser1 = duser1.merge(current_debtAmt, on="uid", how="left")
            duser1["current_MindebtAmt_" + str(window_size)] = duser1["current_MindebtAmt_" + str(window_size)].fillna(
                0.0)
        elif idx == 1:
            pay_date_mask = df["pay_end_date"] > pd.Timestamp(valid_end_date)
            tmp = df[mask & pay_date_mask].reset_index(drop=True)
            tmp["payed_num"] = (pd.Timestamp(test_end_date) - tmp["loan_time"]).apply(
                lambda x: math.ceil(x.days / 30.0))
            tmp["debtAmt"] = tmp["loan_amount"] - tmp["amt_per_plan"] * tmp["payed_num"]
            current_debtAmt = tmp.groupby("uid")["debtAmt"].sum().rename(
                "current_MindebtAmt_" + str(window_size)).reset_index()
            duser2 = duser2.merge(current_debtAmt, on="uid", how="left")
            duser2["current_MindebtAmt_" + str(window_size)] = duser2["current_MindebtAmt_" + str(window_size)].fillna(
                0.0)
    return duser1, duser2
...
def currentMaxDebtAmt(df, duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        if idx == 0:
            ...
            tmp = df[mask & pay_date_mask].reset_index(drop=True)
            tmp["payed_num"] = (pd.Timestamp(valid_end_date) - tmp["loan_time"]).apply(
                lambda x: math.floor(x.days / 30.0))
            tmp["debtAmt"] = tmp["loan_amount"] - tmp["amt_per_plan"] * tmp["payed_num"]
            current_debtAmt = tmp.groupby("uid")["debtAmt"].sum().rename(
                "current_MaxdebtAmt_" + str(window_size)).reset_index()
            duser1 = duser1.merge(current_debtAmt, on="uid", how="left")
            duser1["current_MaxdebtAmt_" + str(window_size)] = duser1["current_MaxdebtAmt_" + str(window_size)].fillna(
                0.0)
        elif idx == 1:
            pay_date_mask = df["pay_end_date"] > pd.Timestamp(valid_end_date)
            tmp = df[mask & pay_date_mask].reset_index(drop=True)
            ...
            current_debtAmt = tmp.groupby("uid")["debtAmt"].sum().rename(
                "current_MaxdebtAmt_" + str(window_size)).reset_index()
            duser2 = duser2.merge(current_debtAmt, on="uid", how="left")
            duser2["current_MaxdebtAmt_" + str(window_size)] = duser2["current_MaxdebtAmt_" + str(window_size)].fillna(
                0.0)
    return duser1, duser2
...
def orderAmtStaus(df, duser, tuser, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            ...
            uid_order_his = tmp.groupby(["uid"])["buy_time_interval"].mean().rename(
                "buy_time_interval_mean").reset_index()
...
def gen_fixed_tw_features_for_click_PidParamUnFold(df, duser1, duser2, col, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        tmp = df[mask].reset_index(drop=True)
        tmp['pidParam'] = pd.Series([str(i) + "_" + str(j) for i, j in zip(list(tmp.pid), list(tmp.param))],
                                    index=tmp.index)
        ...
        uid_pidParam_clicks["pidParam"] = uid_pidParam_clicks['pidParam'].astype(str) + "_pidParamcliks_" + str(
            window_size) + "d"
        uid_pidParam_clicks = uid_pidParam_clicks.pivot(index='uid', columns='pidParam',
                                                        values='pidParam_clicks').reset_index().fillna(0)
        if idx == 0:
        ...
        uid_pidParam_clicks["clicks_pidParam_ratio"] = uid_pidParam_clicks["pidParam_clicks"] / uid_pidParam_clicks[
            "clicks"]
        uid_pidParam_clicks["pidParam"] = uid_pidParam_clicks['pidParam'].astype(str) + "_pidParamcliks_ratio_" + str(
            window_size) + "d"
        uid_pidParam_clicks = uid_pidParam_clicks.pivot(index='uid', columns='pidParam',
                                                        values='clicks_pidParam_ratio').reset_index().fillna(0)
        if idx == 0:
            duser1 = duser1.merge(uid_pidParam_clicks, how="left", on="uid")
        elif idx == 1:
            duser2 = duser2.merge(uid_pidParam_clicks, how="left", on="uid")
...
def gen_fixed_tw_features_for_loan_click_PidParamUnfold(df1, df2, duser1, duser2, col1, col2, window_size):
    ...
    for idx, mask in enumerate([valid_mask_loan, test_mask_loan]):
        ...
        tmp_click['pidParam'] = pd.Series(
            [str(i) + "_" + str(j) for i, j in zip(list(tmp_click.pid), list(tmp_click.param))], index=tmp_click.index)
        uid_cnt_pidparam_by_day = tmp_click.groupby(['uid', 'date', 'pidParam']).click_time.count().rename(
            'cnt_PidParam_in_loan').reset_index()
        ...
        uid_loan_click_tw = uid_loan_click.groupby(['uid', 'pidParam']).cnt_PidParam_in_loan.sum().rename(
            'cnt_PidParam').reset_index()
        uid_loan_click_total = uid_loan_click.groupby('uid').cnt_PidParam_in_loan.sum().rename(
            'cnt_total').reset_index()
        ...
        uid_loan_click_tw['cnt_pidParam_in_loan'] = uid_loan_click_tw['pidParam'].astype(
            str) + "_pidParamcliks_in_loan_" + str(window_size) + "d"
        uid_loan_click_tw['ratio_pidParam_in_loan'] = uid_loan_click_tw['pidParam'].astype(
            str) + "_pidParamRatio_in_loan_" + str(window_size) + "d"
        uid_loan_click_cnt = uid_loan_click_tw.pivot(index='uid', columns='cnt_pidParam_in_loan',
                                                     values='cnt_PidParam').reset_index().fillna(0)
        uid_loan_click_ratio = uid_loan_click_tw.pivot(index='uid', columns='ratio_pidParam_in_loan',
                                                       values='ratio_pidParam').reset_index().fillna(0)
        if idx == 0:
...
def gen_fixed_tw_features_for_notloan_click_PidParamUnfold(df1, df2, duser1, duser2, col1, col2, window_size):
    ...
    for idx, mask in enumerate([valid_mask_loan, test_mask_loan]):
        ...
        tmp_click['pidParam'] = pd.Series(
            [str(i) + "_" + str(j) for i, j in zip(list(tmp_click.pid), list(tmp_click.param))], index=tmp_click.index)
        uid_cnt_pidparam_by_day = tmp_click.groupby(['uid', 'date', 'pidParam']).click_time.count().rename(
            'cnt_PidParam_notin_loan').reset_index()
        uid_cnt_loan_by_day = tmp_loan.groupby(['uid', 'date']).loan_time.count().rename('cnt_loan').reset_index()
        ...
        uid_loan_click_tw = uid_loan_click.groupby(['uid', 'pidParam']).cnt_PidParam_notin_loan.sum().rename(
            'cnt_PidParam').reset_index()
        uid_loan_click_total = uid_loan_click.groupby('uid').cnt_PidParam_notin_loan.sum().rename(
            'cnt_total').reset_index()
        uid_loan_click_tw = uid_loan_click_tw.merge(uid_loan_click_total, how='left', on=['uid'])
        uid_loan_click_tw['ratio_pidParam'] = uid_loan_click_tw.cnt_PidParam / uid_loan_click_tw.cnt_total
        uid_loan_click_tw['cnt_pidParam_notin_loan'] = uid_loan_click_tw['pidParam'].astype(
            str) + "_pidParamcliks_notin_loan_" + str(window_size) + "d"
        uid_loan_click_tw['ratio_pidParam_notin_loan'] = uid_loan_click_tw['pidParam'].astype(
            str) + "_pidParamRatio_notin_loan_" + str(window_size) + "d"
        uid_loan_click_cnt = uid_loan_click_tw.pivot(index='uid', columns='cnt_pidParam_notin_loan',
                                                     values='cnt_PidParam').reset_index().fillna(0)
        uid_loan_click_ratio = uid_loan_click_tw.pivot(index='uid', columns='ratio_pidParam_notin_loan',
                                                       values='ratio_pidParam').reset_index().fillna(0)
        if idx == 0:
            duser1 = duser1.merge(uid_loan_click_cnt, how="left", on="uid")
            duser1 = duser1.merge(uid_loan_click_ratio, how="left", on="uid")
...
def loanClickBehiviorSeries(duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask_loan, test_mask_loan]):
        ...
        tmp = pd.concat([tmp_loan.drop_duplicates(), tmp_click.drop_duplicates()]).groupby(["uid"]).apply(
            lambda x: x.sort_values(["behavior_time"], ascending=True)).reset_index(drop=True)
...
def gen_fixed_tw_features_for_order_cateIdUnFold(df, duser1, duser2, col, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        uid_orders_cnts['cate_id_cnt_ratio_fold'] = uid_orders_cnts['cate_id'].astype(str) + "_cnt_ratio_" + str(
            window_size) + "d"
        uid_orders_amt['cate_id_amt_fold'] = uid_orders_amt['cate_id'].astype(str) + "_amt_" + str(window_size) + "d"
        uid_orders_amt['cate_id_amt_ratio_fold'] = uid_orders_amt['cate_id'].astype(str) + "_amt_ratio_" + str(
            window_size) + "d"
        uid_orders_cnts_cnt = uid_orders_cnts.pivot(index='uid', columns='cate_id_cnt_fold',
                                                    values='cnt_cateId').reset_index().fillna(0)
        uid_orders_cnts_ratio = uid_orders_cnts.pivot(index='uid', columns='cate_id_cnt_ratio_fold',
                                                      values='ratio_cnt_cateId').reset_index().fillna(0)
        uid_orders_amt_amt = uid_orders_amt.pivot(index='uid', columns='cate_id_amt_fold',
                                                  values='sum_cateId').reset_index().fillna(0)
        uid_orders_amt_ratio = uid_orders_amt.pivot(index='uid', columns='cate_id_amt_ratio_fold',
                                                    values='ratio_amt_cateId').reset_index().fillna(0)
...
def gen_fixed_tw_features_for_notOrder_click_PidParamUnfold(df1, df2, duser1, duser2, col1, col2, window_size):
    ...
    for idx, mask in enumerate([valid_mask_order, test_mask_order]):
        ...
        tmp_click['pidParam'] = pd.Series(
            [str(i) + "_" + str(j) for i, j in zip(list(tmp_click.pid), list(tmp_click.param))], index=tmp_click.index)
        uid_cnt_pidparam_by_day = tmp_click.groupby(['uid', 'date', 'pidParam']).click_time.count().rename(
            'cnt_PidParam_notin_order').reset_index()
        ...
        uid_order_click_tw = uid_order_click.groupby(['uid', 'pidParam']).cnt_PidParam_notin_order.sum().rename(
            'cnt_PidParam').reset_index()
        uid_order_click_total = uid_order_click.groupby('uid').cnt_PidParam_notin_order.sum().rename(
            'cnt_total').reset_index()
        ...
        uid_order_click_tw['cnt_pidParam_notin_order'] = uid_order_click_tw['pidParam'].astype(
            str) + "_pidParamcliks_notin_order_" + str(window_size) + "d"
        uid_order_click_tw['ratio_pidParam_notin_order'] = uid_order_click_tw['pidParam'].astype(
            str) + "_pidParamRatio_notin_order_" + str(window_size) + "d"
        uid_order_click_cnt = uid_order_click_tw.pivot(index='uid', columns='cnt_pidParam_notin_order',
                                                       values='cnt_PidParam').reset_index().fillna(0)
        uid_order_click_ratio = uid_order_click_tw.pivot(index='uid', columns='ratio_pidParam_notin_order',
                                                         values='ratio_pidParam').reset_index().fillna(0)
        if idx == 0:
...
def getLoanAmtWithinOrder(duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask_loan, test_mask_loan]):
        ...
        loanAmtWinds = orderLoanAmtWinds.groupby("uid")['loanAmtIn'].sum().rename(
            'loanAmtIn' + str(window_size)).reset_index()
        orderAmtWinds = orderLoanAmtWinds.groupby("uid")['orderAmtIn'].sum().rename(
            'orderAmtIn' + str(window_size)).reset_index()
        orderLoanAmtWinds = loanAmtWinds.merge(orderAmtWinds, how="left", on="uid")
        orderLoanAmtWinds['loanOrderAmtRatioIn' + str(window_size)] = orderLoanAmtWinds[
                                                                          'loanAmtIn' + str(window_size)] / \
                                                                      orderLoanAmtWinds["orderAmtIn" + str(window_size)]
        if idx == 0:
...
def gen_fixed_tw_features_for_click_1m(df, col, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        stat_click_1m_cnt = stat_click_1m.pivot(index='uid', columns='count_1m',
                                                values='click_count').reset_index().fillna(0)
        stat_click_1m_ratio_cnt = stat_click_1m.pivot(index='uid', columns='ratio_cnt_1m',
                                                      values='ratio_cnt').reset_index().fillna(0)
...
def getLoanCntWithinOrder(duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask_loan, test_mask_loan]):
        ...
        orderCntWinds = orderLoanAmtWinds[~orderLoanAmtWinds.loanAmtIn.isnull()].groupby("uid")[
            'orderCnt'].sum().rename('loanOrderCnt' + str(window_size)).reset_index()
        orderTolCnt = orderAmtWinds.groupby("uid")['orderCnt'].sum().rename(
            'orderTolCnt' + str(window_size)).reset_index()
        orderLoanAmtWinds = orderTolCnt.merge(orderCntWinds, how="left", on="uid")
        orderLoanAmtWinds['loanOrderCntRatioIn' + str(window_size)] = orderLoanAmtWinds[
                                                                          'loanOrderCnt' + str(window_size)] / \
                                                                      orderLoanAmtWinds[
                                                                          'orderTolCnt' + str(window_size)]
        if idx == 0:
            duser1 = duser1.merge(orderLoanAmtWinds[["uid", 'loanOrderCntRatioIn' + str(window_size)]], how="left",
                                  on="uid")
        elif idx == 1:
            duser2 = duser2.merge(orderLoanAmtWinds[["uid", 'loanOrderCntRatioIn' + str(window_size)]], how="left",
                                  on="uid")
    return duser1, duser2
...
t_order.loc[t_order["order_amt"] < 0, "order_amt"] = t_order.loc[t_order["order_amt"] < 0, "order_amt"] + t_order.loc[
    t_order["order_amt"] < 0, "discount"]
...
##没有贷款的日子里的点击情况
tr_user, ts_user = gen_fixed_tw_features_for_notloan_click_PidParamUnfold(t_loan, t_click, tr_user, ts_user,
                                                                          'loan_time', "click_time", 30)
tr_user, ts_user = gen_fixed_tw_features_for_notloan_click_PidParamUnfold(t_loan, t_click, tr_user, ts_user,
                                                                          'loan_time', "click_time", 60)
tr_user, ts_user = gen_fixed_tw_features_for_notloan_click_PidParamUnfold(t_loan, t_click, tr_user, ts_user,
                                                                          'loan_time', "click_time", 90)
tr_user, ts_user = gen_fixed_tw_features_for_notloan_click_PidParamUnfold(t_loan, t_click, tr_user, ts_user,
                                                                          'loan_time', "click_time", 15)
...
tr_user, ts_user = gen_fixed_tw_features_for_notOrder_click_PidParamUnfold(t_order, t_click, tr_user, ts_user,
                                                                           'buy_time', "click_time", 30)
tr_user, ts_user = gen_fixed_tw_features_for_notOrder_click_PidParamUnfold(t_order, t_click, tr_user, ts_user,
                                                                           'buy_time', "click_time", 60)
tr_user, ts_user = gen_fixed_tw_features_for_notOrder_click_PidParamUnfold(t_order, t_click, tr_user, ts_user,
                                                                           'buy_time', "click_time", 90)
tr_user, ts_user = gen_fixed_tw_features_for_notOrder_click_PidParamUnfold(t_order, t_click, tr_user, ts_user,
                                                                           'buy_time', "click_time", 15)
