...
def getRemainingLoanAmt(df_loan, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        payingLoan = df_loan_tmp.groupby("uid").apply(
            lambda x: x.sort_values("pay_end_date", ascending=True)).reset_index(drop=True)
        payingLoan["payedLoanAmt"] = payingLoan.groupby("uid")["loan_amount"].cumsum().rename(
            "payedLoanAmt").reset_index(drop=True)
        ...
        if idx == 0:
            ...
            payedAmt = historyLoanAmtLimit.loc[historyLoanAmtLimit["payedAmt"] > 0, ["uid", "payedAmt"]].reset_index(
                drop=True)
...
def encodeByuserProfile(duser, monthlimit):
    duser["encode_key"] = duser["sex_age_limit"].astype(str) + duser["limit_increase"].apply(
        lambda x: round(x, 1)).astype(str)
...
def getConfidenceInterval(duser1, duser2, degree):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        month_loanAmt_status = month_loanAmt.groupby(["uid"])["monthLoanAmt"].agg(
            ["count", "mean", "std"]).reset_index().fillna(0.0)
        month_loanAmt_status["upperConfidence"] = month_loanAmt_status["mean"] + z * month_loanAmt_status["std"] / \
                                                  month_loanAmt_status["count"].apply(lambda x: sqrt(x))
        month_loanAmt_status["lowerConfidence"] = month_loanAmt_status["mean"] - z * month_loanAmt_status["std"] / \
                                                  month_loanAmt_status["count"].apply(lambda x: sqrt(x))
        month_loanAmt_status.columns = ["uid", "loanMonthCount", "loanMonthAmtMean", "loanMonthAmtStd",
...
def getStaytimePid(df_click, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        tmp = df_click[mask].groupby(["uid", "last_click_pid"])["click_interval"].sum().rename(
            "pidStaytime").reset_index()
...
def uidOrderLoanAmtWinds(duser, tuser, window_size):
    ...
    for idx, mask in enumerate([valid_ordermask, test_ordermask]):
        ...
        if idx == 0:
            loanstatus = t_loan[valid_loanmask].groupby(["uid"])["loan_amount"].sum().rename(
                "loanAmt" + str(window_size)).reset_index()
            order_loan = order_status.merge(loanstatus, on="uid", how="left")
            order_loan["loanOrderGapAmt" + str(window_size)] = order_loan["orderAmt" + str(window_size)] - order_loan[
                "loanAmt" + str(window_size)]
            duser = duser.merge(order_loan[["uid", "loanOrderGapAmt" + str(window_size)]], how="left", on='uid')
        elif idx == 1:
            loanstatus = t_loan[test_loanmask].groupby(["uid"])["loan_amount"].sum().rename(
                "loanAmt" + str(window_size)).reset_index()
            order_loan = order_status.merge(loanstatus, on="uid", how="left")
            order_loan["loanOrderGapAmt" + str(window_size)] = order_loan["orderAmt" + str(window_size)] - order_loan[
                "loanAmt" + str(window_size)]
...
def getMonthLoanShiftDiff_WRONG(df_loan, duser1, duser2):
    ...
    uidMonthLoan = uidMonthLoan.groupby(["uid"]).apply(lambda x: x.sort_values(["month"], ascending=True)).reset_index(
        drop=True)
    ...
    duser1 = duser1.merge(uidMonthLoan[uidMonthLoan.month == 10][["uid", "monthLoanAmtDiff", "monthLoanAmtDiff2"]],
                          on="uid", how="left")
    duser2 = duser2.merge(uidMonthLoan[uidMonthLoan.month == 11][["uid", "monthLoanAmtDiff", "monthLoanAmtDiff2"]],
                          on="uid", how="left")
    return duser1, duser2
...
def getNearestLoanOrderInterval(df_loan, df_order, duser1, duser2):
    ...
    for i in [0, 1]:
        ...
        uid_nearest_loan = loan_tmp[
            loan_tmp.groupby(['uid'])['loan_time'].transform(max) == loan_tmp['loan_time']]  # 用户最近一次贷款的时间
        ...
        uid_loan_order["nearest_order_loan_interval"] = (
                uid_loan_order["loan_time"] - uid_loan_order["buy_time"]).apply(lambda x: x.days)
        uid_loan_order["nearest_order_loan_price"] = (uid_loan_order["loan_amount"] - uid_loan_order["order_amt"])
        if i == 0:
            duser1 = duser1.merge(uid_loan_order[["uid", "nearest_order_loan_interval", "nearest_order_loan_price"]],
                                  on="uid", how="left")
        elif i == 1:
            duser2 = duser2.merge(uid_loan_order[["uid", "nearest_order_loan_interval", "nearest_order_loan_price"]],
                                  on="uid", how="left")
    return duser1, duser2
...
def loanAgainBeforePayed(duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask_loan, test_mask_loan]):
        ...
        if idx == 0:
            ...
            stillLoan["topayed_num"] = (stillLoan["pay_end_date"] - pd.Timestamp(valid_end_date)).apply(
            ...
            stillLoan = t_loan[still_mask].reset_index(drop=True)
            stillLoan["topayed_num"] = (stillLoan["pay_end_date"] - pd.Timestamp(test_end_date)).apply(
                lambda x: x.days / 30.0)
...
def loanOrderBehiviorSeries(duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask_loan, test_mask_loan]):
        ...
        tmp = pd.concat([tmp_loan.drop_duplicates(), tmp_order.drop_duplicates()]).groupby(["uid"]).apply(
...
def getOrderLoanAmtdiff(duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        uid_order_click = tmp[tmp["type"] != tmp["last_type"]].groupby("uid")["order_click_interval"].agg(
            ["mean", "max", "min"]).reset_index()
        uid_order_click.columns = ["uid", "mean_order_click_interval", "max_order_click_interval",
                                   "min_order_click_interval"]
        if idx == 0:
...
def getOrderLoanAmtdiff(duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        uid_order_click = tmp[tmp["type"] != tmp["last_type"]].groupby("uid")["order_click_gap"].agg(
            ["mean", "max", "min"]).reset_index()
        uid_order_click.columns = ["uid", "mean_order_click_interval", "max_order_click_interval",
                                   "min_order_click_interval"]
        if idx == 0:
...
def getLoanAmtPerWindows(df_loan, duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        uid_tmp = df_loan_tmp.groupby(["uid", "dayofwindows"])["loan_amount"].sum().rename(
            "loanAmtIn" + str(window_size)).reset_index().groupby("uid")[
            "loanAmtIn" + str(window_size)].mean().reset_index()
        if idx == 0:
...
def getNthNearestLoan(duser1, duser2, pos):
    ...
    for idx, mask in enumerate([loan_validmask, loan_testmask]):
        ...
        if idx == 0:
            t_loan_tmp["pos_" + str(pos) + "_loan_interval"] = (
                    pd.Timestamp(valid_end_date) - t_loan_tmp["loan_time"]).apply(lambda x: x.total_seconds())
            t_loan_tmp["loan_amount_interval_pos_" + str(pos)] = t_loan_tmp["pos_" + str(pos) + "_loan_amount"] / \
                                                                 t_loan_tmp["pos_" + str(pos) + "_loan_interval"]
            t_loan_tmp = t_loan_tmp[["uid", "pos_" + str(pos) + "_loan_amount", "pos_" + str(pos) + "_loan_interval",
                                     "loan_amount_interval_pos_" + str(pos)]]
            duser1 = duser1.merge(t_loan_tmp, on="uid", how="left")
        elif idx == 1:
            t_loan_tmp["pos_" + str(pos) + "_loan_interval"] = (
                    pd.Timestamp(test_end_date) - t_loan_tmp["loan_time"]).apply(lambda x: x.total_seconds())
            t_loan_tmp["loan_amount_interval_pos_" + str(pos)] = t_loan_tmp["pos_" + str(pos) + "_loan_amount"] / \
                                                                 t_loan_tmp["pos_" + str(pos) + "_loan_interval"]
            t_loan_tmp = t_loan_tmp[["uid", "pos_" + str(pos) + "_loan_amount", "pos_" + str(pos) + "_loan_interval",
                                     "loan_amount_interval_pos_" + str(pos)]]
...
def getNthNearestClick(duser1, duser2, pos):
    ...
    for idx, mask in enumerate([click_validmask, click_testmask]):
        ...
        if idx == 0:
            ...
            duser1["pid_param_pos" + str(pos)].cat.categories = np.arange(1, duser1[
                "pid_param_pos" + str(pos)].nunique() + 1)
            ...
            duser2["pid_param_pos" + str(pos)].cat.categories = np.arange(1, duser2[
                "pid_param_pos" + str(pos)].nunique() + 1)
...
def getRealPurchasePower(df_loan, df_order, duser1, duser2):
    ...
    month_order_loan["realPurchasePower"] = month_order_loan["uid_month_orderAmt"] - month_order_loan[
        "uid_month_loanAmt"]
    ...
    tmp.columns = ["uid", "max_realPurchasePower", "min_realPurchasePower", "mean_realPurchasePower",
...
def avgLoanAmt4orderAmt(df_loan, df_order, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        monthOrderLoanTmp["orderLoanAmtGap"] = monthOrderLoanTmp["uid_month_loanAmt"] + monthOrderLoanTmp[
            "uid_month_orderAmt"]
        meanOrderLoanAmtGap = monthOrderLoanTmp.groupby("uid")["orderLoanAmtGap"].mean().rename(
            "meanOrderLoanAmtGap").reset_index()
        if idx == 0:
...
def getOrderStatusAfterNearestLoan(df_loan, df_order, duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        df_loan_tmp = df_loan[mask].reset_index(drop=True)
        maxtime_idx = df_loan_tmp.groupby(['uid'])['loan_time'].transform(max) == df_loan_tmp[
            'loan_time']  # 用户最近一次贷款的情况
        ...
        if idx == 0:
            valid_idx = (order_loan["buy_time"] > order_loan["loan_time"]) & (
                    order_loan["buy_time"] < pd.Timestamp("2016-11-01"))
            ...
            duser1[["afterNearestLoanOrderCnt", "afterNearestLoanOrderAmt"]] = duser1[
                ["afterNearestLoanOrderCnt", "afterNearestLoanOrderAmt"]].fillna(0)
        elif idx == 1:
            valid_idx = (order_loan["buy_time"] > order_loan["loan_time"]) & (
                    order_loan["buy_time"] < pd.Timestamp("2016-12-01"))
            afterLoanOrderStatus = order_loan[valid_idx].groupby("uid")["order_amt"].agg(["count", "sum"]).reset_index()
            afterLoanOrderStatus.columns = ["uid", "afterNearestLoanOrderCnt", "afterNearestLoanOrderAmt"]
            duser2 = duser2.merge(afterLoanOrderStatus, on="uid", how="left")
            duser2[["afterNearestLoanOrderCnt", "afterNearestLoanOrderAmt"]] = duser2[
                ["afterNearestLoanOrderCnt", "afterNearestLoanOrderAmt"]].fillna(0)
    return duser1, duser2
...
def getClickStatusAfterNearestLoan(df_loan, df_click, duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            valid_idx = (click_loan["click_time"] > click_loan["loan_time"]) & (
                    click_loan["click_time"] < pd.Timestamp("2016-11-01"))
            afterLoanOrderStatus = click_loan[valid_idx].groupby("uid")["click_time"].count().rename(
                "afterNearestLoanClickCnt").reset_index()
            duser1 = duser1.merge(afterLoanOrderStatus, on="uid", how="left")
            duser1["afterNearestLoanClickCnt"] = duser1["afterNearestLoanClickCnt"].fillna(0)
        elif idx == 1:
            valid_idx = (click_loan["click_time"] > click_loan["loan_time"]) & (
                    click_loan["click_time"] < pd.Timestamp("2016-12-01"))
            afterLoanOrderStatus = click_loan[valid_idx].groupby("uid")["click_time"].count().rename(
                "afterNearestLoanClickCnt").reset_index()
            duser2 = duser2.merge(afterLoanOrderStatus, on="uid", how="left")
...
def getLoanStatusAfterNearestOrder(df_loan, df_order, duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        maxtime_idx = df_order_tmp.groupby(['uid'])['buy_time'].transform(max) == df_order_tmp[
            'buy_time']  # 用户最近一次贷款的情况
        ...
        if idx == 0:
            valid_idx = (order_loan["buy_time"] <= order_loan["loan_time"]) & (
                    order_loan["loan_time"] < pd.Timestamp("2016-11-01"))
            afterLoanOrderStatus = order_loan[valid_idx].groupby("uid")["loan_amount"].agg(
                ["count", "sum"]).reset_index()
            afterLoanOrderStatus.columns = ["uid", "afterNearestOrderLoanCnt", "afterNearestOrderLoanAmt"]
            duser1 = duser1.merge(afterLoanOrderStatus, on="uid", how="left")
            duser1[["afterNearestOrderLoanCnt", "afterNearestOrderLoanAmt"]] = duser1[
                ["afterNearestOrderLoanCnt", "afterNearestOrderLoanAmt"]].fillna(0)
        elif idx == 1:
            valid_idx = (order_loan["buy_time"] <= order_loan["loan_time"]) & (
                    order_loan["loan_time"] < pd.Timestamp("2016-12-01"))
            afterLoanOrderStatus = order_loan[valid_idx].groupby("uid")["loan_amount"].agg(
                ["count", "sum"]).reset_index()
            afterLoanOrderStatus.columns = ["uid", "afterNearestOrderLoanCnt", "afterNearestOrderLoanAmt"]
            duser2 = duser2.merge(afterLoanOrderStatus, on="uid", how="left")
            duser2[["afterNearestOrderLoanCnt", "afterNearestOrderLoanAmt"]] = duser2[
                ["afterNearestOrderLoanCnt", "afterNearestOrderLoanAmt"]].fillna(0)
    return duser1, duser2
...
def gen_fixed_tw_features_for_order_01(df_order, df_loan, duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            tmp['daysOrder'] = (pd.Timestamp(valid_end_date) - tmp.buy_time).apply(
            ...
            tmp['daysOrder'] = (pd.Timestamp(test_end_date) - tmp.buy_time).apply(
                lambda x: x.days + x.seconds / 86400.0)
            loan_uid = df_loan[loan_test_mask].groupby('uid').loan_amount.sum().rename('loan_amt').reset_index()
        code_cate_id = (tmp[['uid', 'cate_id']].drop_duplicates().merge(loan_uid, how="left", on="uid").groupby(
            'cate_id').loan_amt.sum() / loan_uid.loan_amt.sum()).rename('code_cate_id').reset_index()
        ...
        stat_codeCateId.columns = ['uid'] + [i + '_codeCateId_' + str(window_size) for i in
                                             list(stat_codeCateId.columns)[1:]]
        # 购买类别
        stat_codeId = tmp.groupby('uid').cate_id.agg('nunique').reset_index()
        stat_codeId.columns = ['uid'] + ['nunique' + '_CateId_' + str(window_size) for i in
                                         list(stat_codeId.columns)[1:]]
        # 购买数量的sum
        ...
        stat_order_amt_max.columns = ['uid'] + [i + '_maxAmtOrder_' + str(window_size) for i in
                                                list(stat_order_amt_max.columns)[1:]]
        # 最近的一次的code_cate_id和order_amt
        stat_daysOrder = tmp.groupby('uid').daysOrder.agg(['mean', 'max', 'min']).reset_index()
        stat_daysOrder.columns = ['uid'] + [i + '_daysOrder_' + str(window_size) for i in
                                            list(stat_daysOrder.columns)[1:]]
        cur_order = tmp.groupby('uid').daysOrder.idxmin()
        stat_cur_order = tmp.loc[list(cur_order)][['uid', 'code_cate_id', 'order_amt', 'daysOrder']]
        stat_cur_order.columns = ['uid'] + [i + '_curOrder_' + str(window_size) for i in
                                            list(stat_cur_order.columns)[1:]]
        # 使用的discount的次数和占比
        ...
        stat_Discount.columns = ['uid', 'sum_discount_' + str(window_size), 'sum_orderAmt_' + str(window_size),
                                 'discount_ratio_' + str(window_size)]
        stat = stat_codeCateId.merge(stat_codeId, how="left", on="uid").merge(stat_qty, how="left", on="uid").merge(
            stat_order_amt_max, how="left", on="uid").merge(stat_daysOrder, how="left", on="uid").merge(stat_cur_order,
                                                                                                        how="left",
                                                                                                        on="uid").merge(
            stat_isDiscount, how="left", on="uid").merge(stat_Discount[['uid', 'discount_ratio_' + str(window_size)]],
                                                         how="left", on="uid")
        if idx == 0:
...
def getCatePivotAmtCnt(df, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        months_cate1 = months_cate1.groupby(["uid"]).apply(
...
def codeLoanSeq(df_loan, duser1, duser2):
    ...
    df_loan["loanSeq"] = df_loan["isloan3monthago"].astype(str) + df_loan["isloan2monthago"].astype(str) + df_loan[
        "isloan1monthago"].astype(str)
...
def getMaxLoanAmt(duser, tuser, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        if idx == 0:
            uid_max_loan["maxLoan_interval"] = (pd.Timestamp(valid_end_date) - uid_max_loan["date"]).apply(
                lambda x: x.days)
        elif idx == 1:
            uid_max_loan["maxLoan_interval"] = (pd.Timestamp(test_end_date) - uid_max_loan["date"]).apply(
                lambda x: x.days)
        uid_max_loan["maxloan_price_interval"] = uid_max_loan["maxDailyLoanAmt"] / (
                uid_max_loan["maxLoan_interval"] + 1)
...
def getFixedSexLoan(df, duser1, duser2, window_size):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        ...
        stat_loanAmt.columns = ['sex'] + [i + '_sex' + '_loanAmt_' + str(window_size) + "d" for i in
                                          list(stat_loanAmt.columns)[1:]]
        if idx == 0:
...
def getNthNearestClick(duser1, duser2):
    ...
    for idx, mask in enumerate([click_validmask, click_testmask]):
        ...
        dropcol = ["pid_param_pos" + str(1), "pid_param_pos" + str(2), "pid_param_pos" + str(3),
                   "pid_param_pos" + str(4), "pid_param_pos" + str(5)]
        if idx == 0:
            duser1["pid_param_pos"] = duser1["pid_param_pos" + str(1)] + duser1["pid_param_pos" + str(2)] + duser1[
                "pid_param_pos" + str(3)] + duser1["pid_param_pos" + str(4)] + duser1["pid_param_pos" + str(5)]
            ...
            duser2["pid_param_pos"] = duser2["pid_param_pos" + str(1)] + duser2["pid_param_pos" + str(2)] + duser2[
                "pid_param_pos" + str(3)] + duser2["pid_param_pos" + str(4)] + duser2["pid_param_pos" + str(5)]
...
def get_ordernum_window(df, gkey, window_size):
    ...
    df = df.merge(order_num[["level_1", newcol]], how='left', left_index=True, right_on='level_1').drop(['level_1'],
                                                                                                        axis=1)
    return df
...
param = {'num_leaves': 8, 'num_boost_round': 300, 'objective': 'regression_l2', 'metric': 'rmse', "learning_rate": 0.05,
         "boosting": "gbdt"}
...
for col in set(logfeatures):
    ...
    param = {'num_leaves': 8, 'num_boost_round': 500, 'objective': 'regression_l2', 'metric': 'rmse',
             "learning_rate": 0.05, "boosting": "gbdt", "lambda_l2": 1500, "feature_fraction": 0.9,
             "bagging_fraction": 0.9, "bagging_freq": 50}
