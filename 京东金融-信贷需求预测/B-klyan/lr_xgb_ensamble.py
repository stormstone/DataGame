...
def lgb_create_features(model, lgbdata, split_raw_data):
    ...
    if category_f is not None and len(category_f) > 0:
        law_cate_feature = pd.get_dummies(lgbdata[category_f], sparse=True, columns=category_f).reset_index(
            drop=True)  # 原始分类型特征dummy
        pd_pred_feature = pd.get_dummies(pd_pred_leaf, sparse=True, columns=pd_pred_leaf.columns).reset_index(
            drop=True)  # GBDT叶子dummy
        newdata = pd.concat([split_raw_data.reset_index(drop=True), pd_pred_feature, law_cate_feature], axis=1,
                            ignore_index=True)
        newdata.columns = split_raw_data.columns.append(pd_pred_feature.columns).append(law_cate_feature.columns)
    else:
        print
        "do not contains category features"
        pd_pred_feature = pd.get_dummies(pd_pred_leaf, sparse=True, columns=pd_pred_leaf.columns).reset_index(
            drop=True)  # GBDT叶子dummy
...
def split_raw_data(reg_data, dvalid_data, dtrain_alldata, dtest_data):
    ...
    print
    "loging"
    all_data = reg_data.append(dvalid_data, ignore_index=True).append(dtrain_alldata, ignore_index=True).append(
        dtest_data, ignore_index=True)
    ...
    return tmp, tmp[all_data["type"] == "train"], tmp[all_data["type"] == "valid"], tmp[
        all_data["type"] == "train_all"], tmp[all_data["type"] == "test"]
...
def getPast3MonthLoanPlanInterval(df, duser1, duser2):
    ...
    for idx, mask in enumerate([valid_mask, test_mask]):
        tmp = df[mask].groupby(["uid", "plannum"]).apply(
            lambda x: x.sort_values(["loan_time"], ascending=True)).reset_index(drop=True)
        tmp["last_sameplan_loantime"] = tmp.groupby(["uid", "plannum"])["loan_time"].shift(1)
        tmp["last_sameplan_loan_interval"] = (tmp["loan_time"] - tmp['last_sameplan_loantime']).apply(
            lambda x: x.days + x.seconds / 86400.0)
        ##每期的贷款期数pivot
        perPlanInterval = tmp.groupby(["uid", "plannum"])["last_sameplan_loan_interval"].agg(
            ['max', 'min', 'mean', 'median']).reset_index()
...
category_f = [
    "sex_age_limit"]  # ,u'pid_param_pos1', u'pid_param_pos2',u'pid_param_pos3', u'pid_param_pos4', u'pid_param_pos5']
random.seed(888)
...
param = {'num_leaves': 8, 'num_boost_round': 150, 'objective': 'regression_l2', 'metric': 'rmse', "learning_rate": 0.05,
         "boosting": "gbdt"}
...
all_tmp, split_dtrain, split_dvalid, split_all, split_dtest = split_raw_data(dtrain.data[features],
                                                                             dvalid.data[features],
                                                                             dtrain_all.data[features],
                                                                             dtest.data[features])
...
lr_sub.to_csv("./submission/" + now_time + '_gbdt_lr_Vscore_' + str(rmse_lasso_valid) + '.csv', index=False,
              header=False)
...
ensemble_lr = Lasso(alpha=0.000001, normalize=False, copy_X=True, max_iter=100000, warm_start=True, precompute=True,
                    fit_intercept=False, positive=True)
ensemble_lr.fit(ensemble_train, train_df["loan_sum"])
print
"num_iter: ", ensemble_lr.n_iter_
print
"coef: ", ensemble_lr.coef_
...
now_time = time.strftime("%m-%d %H_%M_%S", time.localtime())
stacking_sub[["uid", "ensemble_pred"]].to_csv("./submission/" + now_time + '_ensemble_pred.csv', index=False,
                                              header=False)
...
now_time = time.strftime("%m-%d %H_%M_%S", time.localtime())
fm_sub[["uid", "loan_sum_fm"]].to_csv("./submission/" + now_time + '_gbdt_fm_Vscore_' + str(rmse_fm_valid) + '.csv',
...
# [m/3]
regr = RandomForestRegressor(n_estimators=100, max_depth=3, criterion="mae", random_state=0, n_jobs=30, oob_score=False,
...
for col in set(z_dvalid.columns) - set(category_f) - set(["sex"]) - set(to_drop):
    ...
    z_dtrain_all[col_zscore] = ((z_dtrain_all[col] - z_dtrain_all[col].mean()) / z_dtrain_all[col].std(ddof=0)).replace(
        [np.inf, -np.inf, np.nan], 0)
    z_dtest[col_zscore] = ((z_dtest[col] - z_dtest[col].mean()) / z_dtest[col].std(ddof=0)).replace(
        [np.inf, -np.inf, np.nan], 0)
    z_dtrain[col_zscore] = ((z_dtrain[col] - z_dtrain_all[col].mean()) / z_dtrain_all[col].std(ddof=0)).replace(
        [np.inf, -np.inf, np.nan], 0)
    z_dvalid[col_zscore] = ((z_dvalid[col] - z_dtrain_all[col].mean()) / z_dtrain_all[col].std(ddof=0)).replace(
        [np.inf, -np.inf, np.nan], 0)
...
lr_zscore_train = pd.concat(
    [lr_train_data[union_feature].reset_index(drop=True), z_dtrain[col_zscore].reset_index(drop=True)], axis=1)
lr_zscore_valid = pd.concat(
    [lr_valid_data[union_feature].reset_index(drop=True), z_dvalid[col_zscore].reset_index(drop=True)], axis=1)
