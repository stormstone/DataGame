...
action_user_type = pd.pivot_table(action_last, index=["userid"], columns=["actionType"], values="before_type_time_gap",
                                  fill_value=100000).reset_index()
data = data.merge(action_user_type, on="userid", how="left")
...
action_user_type = pd.pivot_table(action_56, index=["userid"], columns=["actionType"], values="actionTime_long",
                                  fill_value=100000).reset_index()
data = data.merge(action_user_type, on="userid", how="left")
action_56["actionType"] = action_56["actionType"].apply(lambda x: x.replace("action_long_", "action_gap_"))
action_user_type = pd.pivot_table(action_56, index=["userid"], columns=["actionType"], values="actionTime_gap",
                                  fill_value=100000).reset_index()
data = data.merge(action_user_type, on="userid", how="left")
...
if not os.path.exists("input/string_match.csv"):
    action_str = pd.DataFrame(
        action.groupby("userid").actionType.apply(lambda x: "".join([str(i) for i in list(x)]))).reset_index()
    ...
    for i in [1, 2, 3, 4, 5, 6, 7, 8]:
        ...
        action_str_last["last_%s_search_rt" % i] = action_str_last["last_%s_str" % i].apply(
            lambda x: len(re.findall(x + "8", all_str)) / float(len(re.findall(x, all_str))))
