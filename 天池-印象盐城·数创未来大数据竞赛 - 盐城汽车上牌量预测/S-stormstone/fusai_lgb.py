...
print('resultA：    {}'.format(mean_squared_error(r_testA.cnt, test_preds.mean(axis=1))))
...
plt.xlabel('Date')
plt.ylabel('Cnt')
plt.plot(r_testA.date, r_testA.cnt, 'go')
plt.plot(test_feat.date, test_preds.mean(axis=1), 'ro')
plt.savefig('fusai_result/LgbAnswerA/sub_Lgb{}.png'.format(now_day))
plt.show()
...
submission.to_csv(r'fusai_result/LgbAnswerA/sub_Lgb{}.csv'.format(now_day), header=None,
                  index=False, columns=columns, float_format='%.0f')
submission.to_csv(r'fusai_result/LgbAnswerA/sub_Lgb{}.txt'.format(now_day), header=False, sep='\t',
                  index=False, columns=columns, float_format='%.0f')
...
for i in range(10):
    ...
    plt.plot(testA_brand.date, testA_brand.pred, 'ro')
    plt.savefig('fusai_result/LgbAnswerA/brand{}.png'.format(i + 1))
    plt.show()
    plt.title('brand-%d' % (i + 1))
    plt.xlabel('Date')
    plt.ylabel('Cnt')
    ...
    plt.plot(testA_brand.date, testA_brand.pred, 'r')
    plt.savefig('fusai_result/LgbAnswerA/brand{}_l.png'.format(i + 1))
    plt.show()
'''
'''