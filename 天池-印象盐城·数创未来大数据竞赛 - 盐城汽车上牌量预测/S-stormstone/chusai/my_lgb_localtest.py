...
plt.plot(data_testA_result.date, data_testA_result.cnt, 'y')
...
# submission
'''
plt.savefig('result/sub_myLgb{}_l.png'.format(now_time))
submission.to_csv(r'result/sub_myLgb{}.csv'.format(now_time), header=None,
                  index=False, float_format='%.0f')
submission.to_csv(r'result/sub_myLgb{}.txt'.format(now_time), header=False, sep='\t',
                  index=False, float_format='%.0f')

'''
print('线下得分 resultA：    {}'.format(mean_squared_error(data_testA_result['cnt'], submission['pred'])))
